...
public class CorruptionGen implements IChunkGenerator
{
		//Block that replaces End Stone
		protected static final IBlockState MAIN_BLOCK = ModBlocks.CORRUPTED_COBBLESTONE.getDefaultState();
	    ...
	    public CorruptionGen(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
	    {
	        this.world = world;
	        ...
	        spawnableCreatureList.add(new SpawnListEntry(EntityFormas.class, 1, 1, 1));
	        spawnableCreatureList.add(new SpawnListEntry(EntityGam.class, 1, 1, 1));
	        spawnableCreatureList.add(new SpawnListEntry(EntityMilkshake.class, 1, 1, 1));
	    ...
	    public Chunk generateChunk(int x, int z)
	    {
	        ...
	        {
	            abyte[i] = (byte)Biome.getIdForBiome(BiomeInit.CORRUPTION_BIOME);
	        }
	    ...
	    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
	    {
	        ...
	        for (int i = -12; i <= 12; ++i)
	        {
	            for (int j = -12; j <= 12; ++j)
	            {
	                ...
	                if (k * k + l * l > 4096L && CorruptionGen.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
	                {
