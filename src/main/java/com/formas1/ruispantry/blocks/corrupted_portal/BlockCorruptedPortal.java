package com.formas1.ruispantry.blocks.corrupted_portal;
...
public class BlockCorruptedPortal extends BlockBreakable implements IHasModel
{
	...
	public BlockCorruptedPortal(String name)
	{
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn)
	{
		if(!entityIn.isRiding() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && entityIn instanceof EntityPlayerMP)
		{
			...
			else if(player.dimension != DimensionInit.CORRUPTION.getId())
			{
				player.timeUntilPortal = 10;
				//player.mcServer.getPlayerList().transferPlayerToDimension(player, DimensionInit.CORRUPTION.getId(), new CorruptedTeleporter(player.mcServer.getWorld(DimensionInit.CORRUPTION.getId())));
				player.mcServer.getPlayerList().transferPlayerToDimension(player, DimensionInit.CORRUPTION.getId(), new CustomTeleport(player.mcServer.getWorld(DimensionInit.CORRUPTION.getId()), player.posX, player.posY, player.posZ));
				player.addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 40, 0, false, true));
			}
			else
			{
				player.timeUntilPortal = 10;
				//player.mcServer.getPlayerList().transferPlayerToDimension(player, 0, new CorruptedTeleporter(player.mcServer.getWorld(0)));
				player.mcServer.getPlayerList().transferPlayerToDimension(player, 0, new CustomTeleport(player.mcServer.getWorld(0), player.posX, player.posY, player.posZ));
				player.addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 40, 0, false, true));
			}
		}
	}
	...
	public Size(World world, BlockPos pos, EnumFacing.Axis axis)
	{
		...
		for(BlockPos blockpos = pos; pos.getY() > blockpos.getY() - 21 && pos.getY() > 0 && this.isEmptyBlock(world.getBlockState(pos.down()).getBlock()); pos.down())	{;}
	...
	protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing)
	{
		...
		for(i = 0; i < 22; ++i)
		{
			...
			if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != ModBlocks.SPINEL_BLOCK)	break;
		}
		
		Block block = this.world.getBlockState(pos.offset(facing, i)).getBlock();
		return block == ModBlocks.SPINEL_BLOCK ? i : 0;
	}
	...
	protected int calculatePortalHeight()
	{
		...
		for(this.height = 0; this.height < 21; ++this.height)
		{
			for(int i = 0; i < this.width; ++i)
			{
				...
				if(block == ModBlocks.CORRUPTED_PORTAL)	++this.portalBlockCount;
				
				if(i == 0)
				{
					block = this.world.getBlockState(pos.offset(leftDir)).getBlock();
					if(block != ModBlocks.SPINEL_BLOCK) break label56;
				}
	...
	{
		if(this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != ModBlocks.SPINEL_BLOCK)
		{
			this.height = 0;
			break;
		}
	...
	{
		return block.getDefaultState().getMaterial() == Material.AIR || block == Blocks.FIRE || block == ModBlocks.CORRUPTED_PORTAL;
	}
	...
	public void placePortalBlocks()
	{
		for(int i = 0; i < this.width; ++i)
		{
			...
			for(int j = 0; j < this.height; ++j)	this.world.setBlockState(pos.up(j), ModBlocks.CORRUPTED_PORTAL.getDefaultState().withProperty(BlockCorruptedPortal.AXIS, this.axis), 2);
		}
	}
}
}
