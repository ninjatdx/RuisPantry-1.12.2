...
public class WorldGenTreesCorruptedOak extends WorldGenAbstractTree
{
	 private IBlockState blockStateWood = ModBlocks.CORRUPTED_OAK_LOG.getDefaultState();
	 private IBlockState blockStateLeaves = ModBlocks.CORRUPTED_OAK_LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
	 private final int minTreeHeight = 5;
	 
	  public WorldGenTreesCorruptedOak(boolean parShouldNotify)
	    {
	        ...
	        if (parBlockPos.getY() >= 1 && parBlockPos.getY() + minHeight + 1 <= parWorld.getHeight())
	        {
	            ...
	            else
	            {
	               ...
	               // if (state.getBlock().canSustainPlant(state, parWorld, parBlockPos.down(), EnumFacing.UP, (IPlantable) Blocks.SAPLING) && parBlockPos.getY() < parWorld.getHeight() - minHeight - 1)
	                if (state.getBlock() == ModBlocks.CORRUPTED_GRASS || state.getBlock() == ModBlocks.CORRUPTED_DIRT && parBlockPos.getY() < parWorld.getHeight() - minHeight - 1)
	                {
