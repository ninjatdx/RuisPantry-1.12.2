...
public class CorruptedTeleporter extends Teleporter
{
	protected final WorldServer world;
	protected final Random random;
	protected final Long2ObjectMap<CorruptedTeleporter.Position> cache = new Long2ObjectOpenHashMap<CorruptedTeleporter.Position>(4096);
	
	public CorruptedTeleporter(WorldServer world)
	{
		super(world);
		this.world = world;
	...
	{
		if(this.world.provider.getDimensionType().getId() != 300)
		{
				...
				for(int k1 = -2; k1 <= 2; ++k1)
				{
					for(int l1 = -1; l1 <= 3; ++l1)
					{
						int i2 = i + k1,
							...
							k2 = k - j1;
						boolean flag = l1 < 0;
						this.world.setBlockState(new BlockPos(i2, j2, k2), flag ? ModBlocks.SPINEL_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, float rotationYaw)
	{
			...
			for(int i1 = -128; i1 <= 128; ++i1)
			{
				...
				for(int j1 = -128; j1 <= 128; ++j1)
				{
					for(BlockPos pos1 = pos3.add(i1, this.world.getActualHeight() - 1 - pos3.getY(), j1); pos1.getY() >= 0; pos1 = pos2)
					{
						...
						if(this.world.getBlockState(pos1).getBlock() == ModBlocks.CORRUPTED_PORTAL)
						{
							for(pos2 = pos1.down(); this.world.getBlockState(pos2).getBlock() == ModBlocks.CORRUPTED_PORTAL; pos2 = pos2.down())	pos1 = pos2;
		...
		if(d0 >= 0.0D)
		{
			...
			double d5 = pos.getX() +0.5D,
					d7 = (double)	pos.getZ() + 0.5D;
			PatternHelper helper = ModBlocks.CORRUPTED_PORTAL.createPatternHelper(world, pos);
			...
			entity.motionX = d3 * (double)f2 + d4 * (double)f;
				...
				for(int j7 = -1; j7 <= 1; ++j7)
				{
					for(int l7 = 1; l7 <= 3; ++l7)
					{
						for(int k8 = -1; k8 <= 3; ++k8)
						{
							...
							boolean flag = k8 < 0;
							this.world.setBlockState(new BlockPos(k9, k10, k11), flag ? ModBlocks.SPINEL_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			
			IBlockState state = ModBlocks.CORRUPTED_PORTAL.getDefaultState().withProperty(BlockCorruptedPortal.AXIS, l6 == 0 ? Axis.Z : Axis.X);
			...
			for(int i8 = 0; i8 < 4; ++i8)
			{
				for(int l8 = 0; l8 < 4; ++l8)
				{
					for(int l9 = -1; l9 < 4; ++l9)
					{
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12), flag1 ? ModBlocks.SPINEL_BLOCK.getDefaultState() : state, 2);
					}
				}
